...
public ResponseEntity<CriterioModel> update(@PathVariable("id") Long id, @RequestBody CriterioModel criterioModel) {
    ...
    if (p.isPresent()) {
        var criterio = p.get();
        if ( CriterioModel.getnomeCriterio() != null )
        	criterio.setStatus(CriterioModel.getnomeCriterio());
        if ( CriterioModel.getpontuacaoCriterio() != null )
        	criterio.setStatus(CriterioModel.getpontuacaoCriterio());
        if ( CriterioModel.getprova() != null )
        	criterio.setStatus(CriterioModel.getprova());
